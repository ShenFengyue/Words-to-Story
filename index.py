...
@app.route("/api/word-lookup", methods=["POST", "OPTIONS"])
def word_lookup():
    """查询单词的详细信息（词形变换、常用搭配、常用例句）—— 调 DeepSeek 生成。"""
    if request.method == "OPTIONS":
        return ("", 204)
    ...
    prompt = (
        "你是一个英语词典助手。请为以下英文单词提供词典信息。\n\n"
        f"单词: {word}\n\n"
        "请返回一个 JSON 对象，包含以下字段：\n\n"
        "1. \"word_forms\": 词形变换，列出不同语法形式（名词单复数、动词时态、形容词比较级/最高级等）。"
        "若无可变形式返回空数组。每项含 \"form\"（词形）和 \"cn\"（中文说明）。\n"
        "2. \"collocations\": 常用搭配（3-5个），每项含 \"en\"（英文搭配）和 \"cn\"（中文释义）。\n"
        "3. \"examples\": 常用例句（3-5个），每项含 \"en\"（英文例句）和 \"cn\"（中文翻译）。\n\n"
        "JSON 示例:\n"
        '{"word_forms":[{"form":"running","cn":"现在分词"},{"form":"ran","cn":"过去式"}],'
        '"collocations":[{"en":"run a business","cn":"经营一家公司"}],'
        '"examples":[{"en":"She runs every morning.","cn":"她每天早上跑步。"}]}\n\n'
        "只输出 JSON，不要任何多余内容。"
    )
    ...
    try:
        r = requests.post(
            ...
            json={
                "model": MODEL,
                "messages": [
                    {"role": "system", "content": "你是英语词典助手，只输出 JSON，不含 Markdown 或解释。"},
                    {"role": "user", "content": prompt},
                ],
                "temperature": 0.3,
                "response_format": {"type": "json_object"},
        ...
        if r.status_code != 200:
            return jsonify(error=f"DeepSeek 异常 ({r.status_code})"), 502
