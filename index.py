...
def save_records(arr, sha=None):
    ...
    # 按 seq 降序排列，最新的在前
    arr.sort(key=lambda r: r.get("seq", 0) or 0, reverse=True)
    if not GITHUB_REPO or not GITHUB_TOKEN:
...
def admin_list():
    ...
    # 按 seq 降序（最新在前）
    all_records.sort(key=lambda r: r.get("seq", 0) or 0, reverse=True)
    ...
    for r in all_records:
        ...
        if uid and uid not in all_uids:
            name = r.get("name", "")
            if not name:
                name = load_users().get(uid, "")
            all_uids[uid] = name

    # 应用筛选
    filtered = all_records
    if uid_filter:
        filtered = [r for r in filtered if r.get("uid", "") == uid_filter]
    if rating_filter == "good":
        filtered = [r for r in filtered if r.get("rating") == "good"]
    elif rating_filter == "bad":
        filtered = [r for r in filtered if r.get("rating") == "bad"]
    elif rating_filter == "none":
        filtered = [r for r in filtered if r.get("rating") is None]
