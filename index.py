...
def save_records(arr, sha=None):
    """写回整个记录数组；GitHub 模式下遇到 409 自动重试；本地模式直接写文件。"""
    if _use_github():
    ...
    p = os.path.join(LOCAL_DATA_DIR, "feedback.json")
...
def load_users():
    if _use_github():
        try:
            ...
            return {}
    p = os.path.join(LOCAL_DATA_DIR, "users.json")
    try:
        with open(p, "r", encoding="utf-8") as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
...
def upsert_record(rec):
    ...
    if idx is not None:
        # 更新已有记录：保留原有序号
        rec["seq"] = arr[idx].get("seq", 0)
        arr[idx] = rec
    else:
        # 新增记录：自动分配序号
        max_seq = max((r.get("seq", 0) for r in arr), default=0)
        rec["seq"] = max_seq + 1
        arr.append(rec)
...
def load_comments():
    """返回所有用户评论列表。"""
    arr = load_records()
    return [r.get("comment") for r in arr if r.get("comment", "").strip()]
...
def generate():
    ...
    if comments:
        user_prompt += (
            '用户偏好：以下是用户对自己过往故事的评论，代表了用户的审美倾向和期望，'
            '请在本次生成中尽量呼应这些偏好：\n- '
            + '\n- '.join(comments)
            + '\n'
        )
...
def rate():
    ...
    if rating == "good":
        api_key = os.environ.get("DEEPSEEK_API_KEY")
        if api_key:
            # 如果已有记录且带 feature，复用；否则新生成
            existing = next((r for r in load_records() if r.get("id") == rid), None)
            if existing and existing.get("feature"):
                rec["feature"] = existing["feature"]
            else:
...
def graph_data():
    ...
    return jsonify(records=clean)


# ─── Word Detail ──────────────────────────────────────────────

_WORD_DETAIL_PROMPT = """You are a professional English dictionary for Chinese learners.
Given an English word, explain it thoroughly in Chinese.

Output ONLY a single JSON object (no markdown, no extra text):

{
  "word": "original English word",
  "phonetic": "pronunciation in IPA or simplified Chinese",
  "forms": [
    {"label": "复数", "value": "words"},
    {"label": "过去式", "value": "worded"}
  ],
  "collocations": [
    {"phrase": "in other words", "meaning": "换句话说"}
  ],
  "examples": [
    {"en": "A natural example sentence.", "cn": "对应的中文翻译。"}
  ]
}

Rules:
- forms: list all relevant inflections (plural, past tense, past participle, present participle, comparative, superlative, third-person singular, etc.). If the word has no inflections, return empty array [].
- collocations: 3-5 common phrases or fixed expressions with Chinese meanings. Each phrase should be a natural combination.
- examples: 3-4 natural example sentences with accurate Chinese translations. Sentences should be everyday, useful ones.
- phonetic: prefer IPA notation. If unsure, use simplified Chinese approximate pronunciation.
- All Chinese content (labels, meanings, translations) must be natural and accurate.
"""


@app.route("/api/word-detail", methods=["POST", "OPTIONS"])
def word_detail():
    if request.method == "OPTIONS":
        return ("", 204)
    api_key = os.environ.get("DEEPSEEK_API_KEY")
    if not api_key:
        return jsonify(error="server_missing_deepseek_key"), 500
    data = request.get_json(silent=True) or {}
    ...
    try:
        r = requests.post(
            ...
            json={
                "model": MODEL,
                "messages": [
                    {"role": "system", "content": _WORD_DETAIL_PROMPT},
